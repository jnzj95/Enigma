...
'''
Fix start of save_diverge_frame
'''
Enigma_Fullscreen = False
'-------------------------States-------------------------'
AM_BPM = 0
AM_On_Click = 1
AM_forever = 2
...
saveFrame = False
while True:
    if not np.array_equal(video_capture.read(), last_image):
            ...
            if save_diverge_count >= diverge_max_frame + 1:
                ...
                if diverge_frame_time_interval <= div_timer_after - div_timer_before:
                    if diverge_save_trail:
                        ...
                        combine_diverge_frame = np.zeros((cam_res[1], cam_res[0], 3), dtype=np.uint8)
                        if save_diverge_count == 1 or div_start_frame_count == 1: #1st time through the loop, set a new div_start_timer_market
                            if save_diverge_count == 1:
                                start_marker = div_frame_partial_idx%afterimage_count
                                div_start_timer_marker = time.time()
                            combine_diverge_frame = np.copy(diff_frames[start_marker%afterimage_count])
                        div_timer_after = time.time()
                        if div_timer_after - div_start_timer_marker >= bump_diff_hold_timer:
                            div_start_frame_count += 1
                            div_start_timer_marker = time.time()
                        for k in np.arange(0, div_start_frame_count-1):
                            combine_diverge_frame = cv2.addWeighted(combine_diverge_frame, 1.0, diff_frames[(start_marker+k)%afterimage_count], 1.0, 0)
                            print(start_marker)
    ...
    if state == Line:
        ...
        #line_diff_AM = cv2.absdiff(last_image_3, current_image)
